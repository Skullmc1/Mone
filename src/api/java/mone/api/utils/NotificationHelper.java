...
public class NotificationHelper {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "Mone");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("Mone");
                    tray.add(trayIcon);
                }

                trayIcon.displayMessage("Mone", text, error ? TrayIcon.MessageType.ERROR : TrayIcon.MessageType.INFO);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
    ...
    private static void mac(String text) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + text + "\" with title \"Mone\"");
        try {
    ...
    private static void linux(String text) {
        ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "Mone", text);
        try {
            processBuilder.start();
        } catch (IOException e) {
            e.printStackTrace();
