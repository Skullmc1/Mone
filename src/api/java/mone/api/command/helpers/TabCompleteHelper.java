...
package mone.api.command.helpers;
...
import mone.api.event.events.TabCompleteEvent;
...
public class TabCompleteHelper {
    ...
    public TabCompleteHelper addSettings() {
        return append(
                MoneAPI.getSettings().allSettings.stream()
    ...
    public TabCompleteHelper addModifiedSettings() {
        return append(
                SettingsUtil.modifiedSettings(MoneAPI.getSettings()).stream()
                        .map(Settings.Setting::getName)
                        .sorted(String.CASE_INSENSITIVE_ORDER)
        );
    }
    ...
    public TabCompleteHelper addToggleableSettings() {
        return append(
                MoneAPI.getSettings().getAllValuesByType(Boolean.class).stream()
                        .map(Settings.Setting::getName)
                        .sorted(String.CASE_INSENSITIVE_ORDER)
        );
    }
