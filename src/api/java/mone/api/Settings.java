...
import mone.api.utils.NotificationHelper;
import mone.api.utils.SettingsUtil;
import mone.api.utils.TypeUtils;
import mone.api.utils.gui.MoneToast;
...
/**
 * Mone's settings. Settings apply to all Mone instances.
 *
 */
public final class Settings {
    private static final Logger LOGGER = LoggerFactory.getLogger("Mone");

    /**
     * Allow Mone to break blocks
     */
    public final Setting<Boolean> allowBreak = new Setting<>(true);

    /**
     * Blocks that Mone will be allowed to break even with allowBreak set to false
     */
    public final Setting<List<Block>> allowBreakAnyway = new Setting<>(new ArrayList<>());

    /**
     * Allow Mone to sprint
     */
    public final Setting<Boolean> allowSprint = new Setting<>(true);

    /**
     * Allow Mone to place blocks
     */
    public final Setting<Boolean> allowPlace = new Setting<>(true);

    /**
     * Allow Mone to place blocks in fluid source blocks
     */
    public final Setting<Boolean> allowPlaceInFluidsSource = new Setting<>(true);

    /**
     * Allow Mone to place blocks in flowing fluid
     */
    public final Setting<Boolean> allowPlaceInFluidsFlow = new Setting<>(true);

    /**
     * Allow Mone to move items in your inventory to your hotbar
     */
    ...
    /**
     * Disable Mone's auto-tool at runtime, but still assume that another mod will provide auto tool functionality
     * <p>
     * Specifically, path calculation will still assume that an auto tool will run at execution time, even though
     * Mone itself will not do that.
     */
     ...
     * <p>
     * Decrease to make Mone more often consider paths that would require placing blocks
     */
    ...
    /**
     * Allow Mone to fall arbitrary distances and place a water bucket beneath it.
     * Reliability: questionable.
     */
    public final Setting<Boolean> allowWaterBucketFall = new Setting<>(true);

    /**
     * Allow Mone to assume it can walk on still water just like any other block.
     * This functionality is assumed to be provided by a separate library that might have imported Mone.
     * <p>
    ...
    /**
     * Blocks that Mone is allowed to place (as throwaway, for sneak bridging, pillaring, etc.)
     */
    ...
    /**
     * Blocks that Mone will attempt to avoid (Used in avoidance)
     */
    ...
    /**
     * Blocks that Mone is not allowed to break
     */
    ...
    /**
     * blocks that Mone shouldn't break, but can if it needs to.
     */
     ...
     * <p>
     * Syntax same as <a href="https://Mone.Qceee.com/Mone/api/Settings.html#buildSubstitutes">buildSubstitutes</a>
     */
     ...
     * <p>
     * If a schematic asks for a block on this mapping, Mone will place the first placeable block in the mapped list
     * <p>
     * Usage Syntax:
     * <pre>
    ...
    /**
     * If this setting is true, Mone will never break a block that is adjacent to an unsupported falling block.
     * <p>
     ...
     * pathing scenarios. And they can cause Mone to get trapped indefinitely in a strange scenario.
     * <p>
     ...
     *
     * @see <a href="https://github.com/cabaletta/Mone/issues/18">Issue #18</a>
     */
     ...
     *
     * @see <a href="https://github.com/cabaletta/Mone/issues/114">Issue #114</a>
     */
    ...
    /**
     * allows Mone to save bed waypoints when interacting with beds
     */
    public final Setting<Boolean> doBedWaypoints = new Setting<>(true);

    /**
     * allows Mone to save death waypoints
     */
     ...
     *
     * @see <a href="https://github.com/cabaletta/Mone/issues/248">Issue #248</a>
     */
    ...
    /**
     * Allow chat based control of Mone. Most likely should be disabled when Mone is imported for use in
     * something else
     */
    ...
    /**
     * Will cause some minor behavioral differences to ensure that Mone works on anticheats.
     * <p>
    ...
    /**
     * 😎 Render cached chunks as semitransparent. Doesn't work with OptiFine 😭 Rarely randomly crashes, see <a href="https://github.com/cabaletta/Mone/issues/327">this issue</a>.
     * <p>
    ...
    /**
     * Whether or not to allow you to run Mone commands with the prefix
     */
    ...
    /**
     * Use a short Mone prefix [B] instead of [Mone] when logging to chat
     */
    ...
    /**
     * This will only allow Mone to mine exposed ores, can be used to stop ore obfuscators on servers that use them.
     */
    ...
    /**
     * Build in map art mode, which makes Mone only care about the top block in each column
     */
     ...
     * <p>
     * For example, if Mone is doing "mine iron_ore", the instant it breaks the ore (and it becomes air), that location
     ...
     * <p>
     * However, simply discarding old chunks because they are old is inadvisable. Mone is extremely good at correcting
     * itself and its paths as it learns new information, as new chunks load. There is no scenario in which having an
     * incorrect cache can cause Mone to get stuck, take damage, or perform any action it wouldn't otherwise, everything
     ...
     * before they're in render distance. In fact, when the chunkCaching setting is disabled and Mone starts anew
    ...
    /**
     * The function that is called when Mone will log to chat. This function can be added to
    ...
    public final Setting<Consumer<Component>> logger = new Setting<>((msg) -> {
        try {
            ...
            if (mc == null || mc.gui == null) {
                LOGGER.info("[Mone] " + msg.getString());
                return;
            }
            final ChatComponent chat = mc.gui.hud.getChat();
            if (tag != null) {
    ...
    /**
     * The function that is called when Mone will send a desktop notification. This function can be added to
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via setting
     * {@link Setting#value};
     */
    @JavaOnly
    ...
    /**
     * The function that is called when Mone will show a toast. This function can be added to
     * via {@link Consumer#andThen(Consumer)} or it can completely be overriden via setting
     * {@link Setting#value};
     */
    @JavaOnly
    public final Setting<BiConsumer<Component, Component>> toaster = new Setting<>(MoneToast::addOrUpdate);
