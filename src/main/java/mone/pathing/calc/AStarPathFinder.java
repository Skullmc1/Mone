...
import mone.pathing.calc.openset.BinaryHeapOpenSet;
import mone.pathing.movement.CalculationContext;
import mone.pathing.movement.Moves;
import mone.utils.pathing.BetterWorldBorder;
import mone.utils.pathing.Favoring;
import mone.utils.pathing.MutableMoveResult;
...
public final class AStarPathFinder extends AbstractNodeCostSearch {
    ...
    protected Optional<IPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        boolean slowPath = MoneAPI.getSettings().slowPath.value;
        if (slowPath) {
            logDebug("slowPath is on, path timeout will be " + MoneAPI.getSettings().slowPathTimeoutMS.value + "ms instead of " + primaryTimeout + "ms");
        }
        long primaryTimeoutTime = startTime + (slowPath ? MoneAPI.getSettings().slowPathTimeoutMS.value : primaryTimeout);
        long failureTimeoutTime = startTime + (slowPath ? MoneAPI.getSettings().slowPathTimeoutMS.value : failureTimeout);
        ...
        int pathingMaxChunkBorderFetch = MoneAPI.getSettings().pathingMaxChunkBorderFetch.value; // grab all settings beforehand so that changing settings during pathing doesn't cause a crash or unpredictable behavior
        double minimumImprovement = MoneAPI.getSettings().minimumImprovementRepropagation.value ? MIN_IMPROVEMENT : 0;
        ...
        while (!openSet.isEmpty() && numEmptyChunk < pathingMaxChunkBorderFetch && !cancelRequested) {
            ...
            if (slowPath) {
                try {
                    Thread.sleep(MoneAPI.getSettings().slowPathTimeDelayMS.value);
