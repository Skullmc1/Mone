...
public abstract class Movement implements IMovement, MovementHelper {
    ...
    protected Movement(IMone Mone, BetterBlockPos src, BetterBlockPos dest, BetterBlockPos[] toBreak, BetterBlockPos toPlace) {
        this.Mone = Mone;
    ...
    protected Movement(IMone Mone, BetterBlockPos src, BetterBlockPos dest, BetterBlockPos[] toBreak) {
        this(Mone, src, dest, toBreak, null);
    }
    ...
    protected boolean playerInValidPosition() {
        return getValidPositions().contains(ctx.playerFeet()) || getValidPositions().contains(((PathingBehavior) Mone.getPathingBehavior()).pathStart());
    }
    ...
    public MovementStatus update() {
        ...
        currentState.getTarget().getRotation().ifPresent(rotation ->
                Mone.getLookBehavior().updateTarget(
                        rotation,
                        currentState.getTarget().hasToForceRotations()));
        Mone.getInputOverrideHandler().clearAllKeys();
        currentState.getInputStates().forEach((input, forced) -> {
            Mone.getInputOverrideHandler().setInputForceState(input, forced);
        });
    ...
    protected boolean prepared(MovementState state) {
        ...
        for (BetterBlockPos blockPos : positionsToBreak) {
            if (!ctx.world().getEntitiesOfClass(FallingBlockEntity.class, new AABB(0, 0, 0, 1, 1.1, 1).move(blockPos)).isEmpty() && MoneAPI.getSettings().pauseMiningForFallingBlocks.value) {
                return false;
            }
