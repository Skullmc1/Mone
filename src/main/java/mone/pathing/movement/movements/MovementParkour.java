...
public class MovementParkour extends Movement {
    ...
    private MovementParkour(IMone Mone, BetterBlockPos src, int dist, Direction dir, boolean ascend) {
        super(Mone, src, src.relative(dir, dist).above(ascend ? 1 : 0), EMPTY, src.relative(dir, dist).below(ascend ? 0 : 1));
        this.direction = dir;
        this.dist = dist;
        this.ascend = ascend;
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (MoneAPI.getSettings().allowWalkOnMagmaBlocks.value && ctx.world().getBlockState(ctx.playerFeet().below()).is(Blocks.MAGMA_BLOCK)) {
        ...
        if (ctx.playerFeet().equals(dest)) {
            ...
            if (ctx.playerFeet().equals(src.relative(direction)) || ctx.player().position().y - src.y > 0.0001) {
                if (MoneAPI.getSettings().allowPlace.value // see PR #3775
                        && ((Mone) Mone).getInventoryBehavior().hasGenericThrowaway()
                        && !MovementHelper.canWalkOn(ctx, dest.below())
                        && !ctx.player().onGround()
                        && MovementHelper.attemptToPlaceABlock(state, Mone, dest.below(), true, false) == PlaceResult.READY_TO_PLACE
                ) {
