...
public class MovementDiagonal extends Movement {
    ...
    public MovementDiagonal(IMone Mone, BetterBlockPos start, Direction dir1, Direction dir2, int dy) {
        this(Mone, start, start.relative(dir1), start.relative(dir2), dir2, dy);
        // super(start, start.offset(dir1).offset(dir2), new BlockPos[]{start.offset(dir1), start.offset(dir1).up(), start.offset(dir2), start.offset(dir2).up(), start.offset(dir1).offset(dir2), start.offset(dir1).offset(dir2).up()}, new BlockPos[]{start.offset(dir1).offset(dir2).down()});
    }

    private MovementDiagonal(IMone Mone, BetterBlockPos start, BetterBlockPos dir1, BetterBlockPos dir2, Direction drr2, int dy) {
        this(Mone, start, dir1.relative(drr2).above(dy), dir1, dir2);
    }

    private MovementDiagonal(IMone Mone, BetterBlockPos start, BetterBlockPos end, BetterBlockPos dir1, BetterBlockPos dir2) {
        super(Mone, start, end, new BetterBlockPos[]{dir1, dir1.above(), dir2, dir2.above(), end, end.above()});
    }
    ...
    private boolean sprint() {
        if (MovementHelper.isLiquid(ctx, ctx.playerFeet()) && !MoneAPI.getSettings().sprintInWater.value) {
            return false;
        }
        for (int i = 0; i < 4; i++) {
