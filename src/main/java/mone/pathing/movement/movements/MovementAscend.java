...
public class MovementAscend extends Movement {
    ...
    public MovementAscend(IMone Mone, BetterBlockPos src, BetterBlockPos dest) {
        super(Mone, src, dest, new BetterBlockPos[]{dest, src.above(2), dest.above()}, dest.below());
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (!MovementHelper.canWalkOn(ctx, positionToPlace, jumpingOnto)) {
            ...
            if (MovementHelper.attemptToPlaceABlock(state, Mone, dest.below(), false, true) == PlaceResult.READY_TO_PLACE) {
        ...
        state.setInput(Input.SNEAK, MoneAPI.getSettings().allowWalkOnMagmaBlocks.value && jumpingOnto.is(Blocks.MAGMA_BLOCK));
        ...
        if (MoneAPI.getSettings().assumeStep.value || ctx.playerFeet().equals(src.above())) {
