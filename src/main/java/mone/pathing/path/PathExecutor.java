...
import mone.behavior.PathingBehavior;
import mone.pathing.calc.AbstractNodeCostSearch;
...
import mone.pathing.movement.movements.*;
...
import static mone.api.pathing.movement.MovementStatus.*;
...
public class PathExecutor implements IPathExecutor, Helper {
     ...
     *
     * @see <a href="https://github.com/cabaletta/Mone/issues/102">Issue #102</a>
    ...
    public boolean onTick() {
        ...
        if (pathPosition < path.movements().size() - 1) {
            ...
            if (!behavior.Mone.bsi.worldContainsLoadedChunk(next.getDest().x, next.getDest().z)) {
        ...
        if (costEstimateIndex == null || costEstimateIndex != pathPosition) {
            ...
            for (int i = 1; i < MoneAPI.getSettings().costVerificationLookahead.value && pathPosition + i < path.length() - 1; i++) {
        ...
        if (!movement.calculatedWhileLoaded() && currentCost - currentMovementOriginalCostEstimate > MoneAPI.getSettings().maxCostIncrease.value && canCancel) {
        ...
        if (movementStatus == SUCCESS) {
            ...
            ticksOnCurrent++;
            if (ticksOnCurrent > currentMovementOriginalCostEstimate + MoneAPI.getSettings().movementTimeoutTicks.value) {
    ...
    private boolean shouldSprintNextTick() {
        boolean requested = behavior.Mone.getInputOverrideHandler().isInputForcedDown(Input.SPRINT);

        // we'll take it from here, no need for minecraft to see we're holding down control and sprint for us
        behavior.Mone.getInputOverrideHandler().setInputForceState(Input.SPRINT, false);

        // first and foremost, if allowSprint is off, or if we don't have enough hunger, don't try and sprint
        if (!new CalculationContext(behavior.Mone, false).canSprint) {
            return false;
        }
        ...
        if (current instanceof MovementTraverse && pathPosition < path.length() - 3) {
            ...
            if (next instanceof MovementAscend && sprintableAscend(ctx, (MovementTraverse) current, (MovementAscend) next, path.movements().get(pathPosition + 2))) {
                if (skipNow(ctx, current)) {
                    ...
                    onChangeInPathPosition();
                    onTick();
                    behavior.Mone.getInputOverrideHandler().setInputForceState(Input.JUMP, true);
                    return true;
                } else {
            ...
            if (pathPosition < path.length() - 2) {
                ...
                if (MovementHelper.canUseFrostWalker(ctx, next.getDest().below())) {
                    ...
                    if (next instanceof MovementTraverse || next instanceof MovementParkour) {
                        boolean couldPlaceInstead = MoneAPI.getSettings().allowPlace.value && behavior.Mone.getInventoryBehavior().hasGenericThrowaway() && next instanceof MovementParkour; // traverse doesn't react fast enough
        ...
        if (current instanceof MovementAscend && pathPosition != 0) {
            ...
            if (prev instanceof MovementDescend && prev.getDirection().above().equals(current.getDirection().below())) {
                ...
                if (ctx.player().position().y >= center.getY() - 0.07) {
                    behavior.Mone.getInputOverrideHandler().setInputForceState(Input.JUMP, false);
                    return true;
                }
            }
        ...
        if (current instanceof MovementFall) {
            ...
            if (data != null) {
                ...
                clearKeys();
                behavior.Mone.getLookBehavior().updateTarget(RotationUtils.calcRotationFromVec3d(ctx.playerHead(), data.first(), ctx.playerRotations()), false);
                behavior.Mone.getInputOverrideHandler().setInputForceState(Input.MOVE_FORWARD, true);
                return true;
            }
        }
        return false;
    ...
    private static boolean sprintableAscend(IPlayerContext ctx, MovementTraverse current, MovementAscend next, IMovement nextnext) {
        if (!MoneAPI.getSettings().sprintAscends.value) {
            return false;
        }
    ...
    private static boolean canSprintFromDescendInto(IPlayerContext ctx, IMovement current, IMovement next) {
        ...
        return next instanceof MovementDiagonal && MoneAPI.getSettings().allowOvershootDiagonalDescend.value;
    }
    ...
    private void clearKeys() {
        ...
        behavior.Mone.getInputOverrideHandler().clearAllKeys();
    }

    private void cancel() {
        clearKeys();
        behavior.Mone.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
    ...
    private PathExecutor cutIfTooLong() {
        if (pathPosition > MoneAPI.getSettings().maxPathHistoryLength.value) {
            int cutoffAmt = MoneAPI.getSettings().pathHistoryCutoffAmount.value;
