...
import mone.api.command.datatypes.ItemById;
...
public class PickupCommand extends Command {
    ...
    public PickupCommand(IMone Mone) {
        super(Mone, "pickup");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (collecting.isEmpty()) {
            Mone.getFollowProcess().pickup(stack -> true);
            logDirect("Picking up all items");
        } else {
            Mone.getFollowProcess().pickup(stack -> collecting.contains(stack.getItem()));
