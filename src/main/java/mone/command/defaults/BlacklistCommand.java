...
public class BlacklistCommand extends Command {
    ...
    public BlacklistCommand(IMone Mone) {
        super(Mone, "blacklist");
    }

    @Override
    public void execute(String label, IArgConsumer args) throws CommandException {
        args.requireMax(0);
        IGetToBlockProcess proc = Mone.getGetToBlockProcess();
        if (!proc.isActive()) {
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "While going to a block this command blacklists the closest block so that Mone won't attempt to get to it.",
                "",
                "Usage:",
                "> blacklist"
        );
