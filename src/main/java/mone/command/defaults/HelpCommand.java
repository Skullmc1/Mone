...
import mone.api.command.exception.CommandNotFoundException;
...
public class HelpCommand extends Command {
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (!args.hasAny() || args.is(Integer.class)) {
            Paginator.paginate(
                    args, new Paginator<>(
                            this.Mone.getCommandManager().getRegistry().descendingStream()
                                    .filter(command -> !command.hiddenFromHelp())
                                    .collect(Collectors.toList())
                    ),
                    () -> logDirect("All Mone commands (clickable):"),
                    command -> {
            ...
            ICommand command = this.Mone.getCommandManager().getCommand(commandName);
            if (command == null) {
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "Using this command, you can view detailed help information on how to use certain commands of Mone.",
                "",
                "Usage:",
