...
import mone.api.command.datatypes.ForAxis;
import mone.api.command.datatypes.ForBlockOptionalMeta;
import mone.api.command.datatypes.ForDirection;
import mone.api.command.datatypes.RelativeBlockPos;
...
import mone.api.schematic.mask.shape.CylinderMask;
import mone.api.schematic.mask.shape.SphereMask;
import mone.api.selection.ISelection;
import mone.api.selection.ISelectionManager;
import mone.api.utils.BetterBlockPos;
import mone.api.utils.BlockOptionalMeta;
import mone.api.utils.BlockOptionalMetaLookup;
import mone.utils.BlockStateInterface;
import mone.utils.IRenderer;
import mone.utils.schematic.StaticSchematic;
...
public class SelCommand extends Command {
    ...
    private ISelectionManager manager = Mone.getSelectionManager();
    private BetterBlockPos pos1 = null;
    private ISchematic clipboard = null;
    private Vec3i clipboardOffset = null;

    public SelCommand(IMone Mone) {
        super(Mone, "sel", "selection", "s");
        Mone.getGameEventHandler().registerEventListener(new AbstractGameEventListener() {
            @Override
            public void onRenderPass(RenderEvent event) {
                if (!MoneAPI.getSettings().renderSelectionCorners.value || pos1 == null) {
                    return;
                }
                Color color = MoneAPI.getSettings().colorSelectionPos1.value;
                float opacity = MoneAPI.getSettings().selectionOpacity.value;
                float lineWidth = MoneAPI.getSettings().selectionLineWidth.value;
                boolean ignoreDepth = MoneAPI.getSettings().renderSelectionIgnoreDepth.value;
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.POS1 || action == Action.POS2) {
            ...
            Mone.getBuilderProcess().build("Fill", composite, origin);
            ...
            Mone.getBuilderProcess().build("Fill", clipboard, pos.offset(clipboardOffset));
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The sel command allows you to manipulate Mone's selections, similarly to WorldEdit.",
                "",
