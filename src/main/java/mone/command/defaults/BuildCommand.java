...
public class BuildCommand extends Command {
    ...
    public BuildCommand(IMone Mone) {
        super(Mone, "build");
        this.schematicsDir = new File(Mone.getPlayerContext().minecraft().gameDirectory, "schematics");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (FilenameUtils.getExtension(file.getAbsolutePath()).isEmpty()) {
            file = new File(file.getAbsolutePath() + "." + MoneAPI.getSettings().schematicFallbackExtension.value);
        }
        if (!file.exists()) {
            if (file0.exists()) {
        ...
        boolean success = Mone.getBuilderProcess().build(file.getName(), file, buildOrigin);
        if (!success) {
