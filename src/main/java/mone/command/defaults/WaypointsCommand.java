...
public class WaypointsCommand extends Command {
    ...
    public WaypointsCommand(IMone Mone) {
        super(Mone, "waypoints", "waypoint", "wp");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.LIST) {
            ...
            IWaypoint[] waypoints = tag != null
                    ? ForWaypoints.getWaypointsByTag(this.Mone, tag)
                    : ForWaypoints.getWaypoints(this.Mone);
            ...
            ForWaypoints.waypoints(this.Mone).addWaypoint(waypoint);
            ...
            IWaypoint[] waypoints = ForWaypoints.getWaypointsByTag(this.Mone, tag);
            for (IWaypoint waypoint : waypoints) {
                ForWaypoints.waypoints(this.Mone).removeWaypoint(waypoint);
            }
            deletedWaypoints.computeIfAbsent(Mone.getWorldProvider().getCurrentWorld(), k -> new ArrayList<>()).addAll(Arrays.<IWaypoint>asList(waypoints));
            ...
            List<IWaypoint> deletedWaypoints = this.deletedWaypoints.getOrDefault(Mone.getWorldProvider().getCurrentWorld(), Collections.emptyList());
            ...
            waypoints.forEach(ForWaypoints.waypoints(this.Mone)::addWaypoint);
            ...
            if (waypoint == null) {
                ...
                if (action == Action.INFO) {
                    ...
                    recreateComponent.setStyle(recreateComponent.getStyle().withClickEvent(new ClickEvent.SuggestCommand(
                            String.format(
                                    "%s%s save %s %s %s %s %s",
                                    MoneAPI.getSettings().prefix.value, // This uses the normal prefix because it is run by the user.
                                    label,
                                    waypoint.getTag().getName(),
                                    waypoint.getName(),
                    ...
                    ForWaypoints.waypoints(this.Mone).removeWaypoint(waypoint);
                    deletedWaypoints.computeIfAbsent(Mone.getWorldProvider().getCurrentWorld(), k -> new ArrayList<>()).add(waypoint);
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The waypoint command allows you to manage Mone's waypoints.",
                "",
