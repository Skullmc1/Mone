...
package mone.process.elytra;

import mone.api.MoneAPI;
import mone.Mone;
import mone.api.Settings;
import mone.api.behavior.look.IAimProcessor;
import mone.api.behavior.look.ITickableAimProcessor;
...
import mone.process.ElytraProcess;
import mone.utils.BlockStateInterface;
import mone.utils.IRenderer;
import mone.utils.PathRenderer;
import mone.utils.accessor.IFireworkRocketEntity;
...
import static mone.utils.MoneMath.fastCeil;
import static mone.utils.MoneMath.fastFloor;
...
public final class ElytraBehavior implements Helper {
    ...
    public ElytraBehavior(Mone Mone, ElytraProcess process, BlockPos destination, boolean appendDestination) {
        this.Mone = Mone;
        ...
        this.context = new NetherPathfinderContext(MoneAPI.getSettings().elytraNetherSeed.value);
                    ...
                    .whenComplete((result, ex) -> {
                        ...
                        if (ex != null) {
                            ...
                            if (cause instanceof PathCalculationException) {
                                ...
                                if (ctx.player().distanceToSqr(new Vec3(pathStart.getX() + 0.5, pathStart.getY() + 0.5, pathStart.getZ() + 0.5)) < 16 * 16) {
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        final Settings settings = MoneAPI.getSettings();
        ...
        if (!this.blockedLines.isEmpty() && MoneAPI.getSettings().elytraRenderRaytraces.value) {
        ...
        if (this.simulationLine != null && MoneAPI.getSettings().elytraRenderSimulation.value) {
    ...
    public void onReceivePacket(PacketEvent event) {
        if (event.getPacket() instanceof ClientboundPlayerPositionPacket) {
            ctx.minecraft().execute(() -> {
                this.remainingSetBackTicks = MoneAPI.getSettings().elytraFireworkSetbackUseDelay.value;
            });
        }
    }

    public void pathTo() {
        if (!MoneAPI.getSettings().elytraAutoJump.value || ctx.player().isFallFlying()) {
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > MoneAPI.getSettings().elytraTimeBetweenCacheCullSecs.value) {
            this.context.queueCacheCulling(ctx.player().chunkPosition().x(), ctx.player().chunkPosition().z(), MoneAPI.getSettings().elytraCacheCullDistance.value, this.boi);
    ...
    /**
     * Called by {@link Mone.process.ElytraProcess#onTick(boolean, boolean)} when the process is in control and the player is flying
     */
    public void tick() {
        ...
        Mone.getLookBehavior().updateTarget(solution.rotation, false);
        ...
        for (int relaxation = 0; relaxation < 3; relaxation++) { // try for a strict solution first, then relax more and more (if we're in a corner or near some blocks, it will have to relax its constraints a bit)
            ...
            for (int i = Math.min(playerNear + 20, path.size() - 1); i >= minStep; i--) {
                ...
                for (final Pair<Vec3, Integer> candidate : candidates) {
                    ...
                    final double minAvoidance = MoneAPI.getSettings().elytraMinimumAvoidance.value;
    ...
    private void tickUseFireworks(final Vec3 start, final Vec3 goingTo, final boolean isBoosted, final boolean forceUseFirework) {
        ...
        final boolean useOnDescend = !MoneAPI.getSettings().elytraConserveFireworks.value || ctx.player().position().y < goingTo.y + 5;
        ...
        final double elytraFireworkSpeed = MoneAPI.getSettings().elytraFireworkSpeed.value;
        if (this.remainingFireworkTicks <= 0 && (forceUseFirework || (!isBoosted
            ...
            if (!Mone.getInventoryBehavior().throwaway(true, ElytraBehavior::isBoostingFireworks) &&
                    !Mone.getInventoryBehavior().throwaway(true, ElytraBehavior::isFireworks)) {
    ...
    private final class SolverContext {
        ...
        public SolverContext(boolean async) {
            ...
            ITickableAimProcessor aim = ElytraBehavior.this.Mone.getLookBehavior().getAimProcessor().fork();
            if (async) {
    ...
    private boolean isHitboxClear(final SolverContext context, final Vec3 dest, final Double growAmount) {
        ...
        if (MoneAPI.getSettings().elytraRenderHitboxRaytraces.value) {
            boolean clear = true;
            for (int i = 0; i < 8; i++) {
    ...
    public boolean clearView(Vec3 start, Vec3 dest, boolean ignoreLava) {
        ...
        if (MoneAPI.getSettings().elytraRenderRaytraces.value) {
    ...
    private static FloatArrayList pitchesToSolveFor(final float goodPitch, final boolean desperate) {
        final float minPitch = desperate ? -90 : Math.max(goodPitch - MoneAPI.getSettings().elytraPitchRange.value, -89);
        final float maxPitch = desperate ? 90 : Math.min(goodPitch + MoneAPI.getSettings().elytraPitchRange.value, 89);
    ...
    private Pair<Float, Boolean> solvePitch(final SolverContext context, final Vec3 goal, final int relaxation) {
        ...
        final int ticks = desperate ? 3 : context.boost.isBoosted() ? Math.max(5, context.boost.getGuaranteedBoostTicks()) : MoneAPI.getSettings().elytraSimulationTicks.value;
    ...
    private void tickInventoryTransactions() {
        if (invTickCountdown <= 0) {
            ...
            if (r != null) {
                r.run();
                invTickCountdown = MoneAPI.getSettings().ticksBetweenInventoryMoves.value;
            }
        }
    ...
    private int findGoodElytra() {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (slot.getItem() == Items.ELYTRA && (slot.getMaxDamage() - slot.getDamageValue()) > MoneAPI.getSettings().elytraMinimumDurability.value) {
                return i;
            }
        }
        return -1;
    ...
    private void trySwapElytra() {
        if (!MoneAPI.getSettings().elytraAutoSwap.value || !invTransactionQueue.isEmpty()) {
            return;
        }

        ItemStack chest = ctx.player().getItemBySlot(EquipmentSlot.CHEST);
        if (chest.getItem() != Items.ELYTRA
                || chest.getMaxDamage() - chest.getDamageValue() > MoneAPI.getSettings().elytraMinimumDurability.value) {
            return;
        }
    ...
    void logVerbose(String message) {
        if (MoneAPI.getSettings().elytraChatSpam.value) {
            logDebug(message);
        }
    }
}
