...
import mone.api.process.IGetToBlockProcess;
...
public final class GetToBlockProcess extends MoneProcessHelper implements IGetToBlockProcess {
    ...
    public GetToBlockProcess(Mone Mone) {
        super(Mone);
    }
    ...
    public synchronized PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (knownLocations.isEmpty()) {
            if (MoneAPI.getSettings().exploreForBlocks.value && !calcFailed) {
        ...
        if (calcFailed) {
            if (MoneAPI.getSettings().blacklistClosestOnFailure.value) {
        ...
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            ...
            Mone.getExecutor().execute(() -> rescan(current, context));
        }
        if (goal.isInGoal(ctx.playerFeet()) && goal.isInGoal(Mone.getPathingBehavior().pathStart()) && isSafeToCancel) {
            // we're there
    ...
    public class GetToBlockCalculationContext extends CalculationContext {
        ...
        public GetToBlockCalculationContext(boolean forUseOnAnotherThread) {
            super(GetToBlockProcess.super.Mone, forUseOnAnotherThread);
        }
    ...
    private Goal createGoal(BlockPos pos) {
        ...
        if (blockOnTopMustBeRemoved(gettingTo.getBlock()) && MovementHelper.isBlockNormalCube(Mone.bsi.get0(pos.above()))) { // TODO this should be the check for chest openability
    ...
    private boolean rightClick() {
        for (BlockPos pos : knownLocations) {
            ...
            if (reachable.isPresent()) {
                Mone.getLookBehavior().updateTarget(reachable.get(), true);
                if (knownLocations.contains(ctx.getSelectedBlock().orElse(null))) {
                    Mone.getInputOverrideHandler().setInputForceState(Input.CLICK_RIGHT, true); // TODO find some way to right click even if we're in an ESC menu
    ...
    private boolean walkIntoInsteadOfAdjacent(Block block) {
        if (!MoneAPI.getSettings().enterPortal.value) {
            return false;
        }
    ...
    private boolean rightClickOnArrival(Block block) {
        if (!MoneAPI.getSettings().rightClickContainerOnArrival.value) {
            return false;
        }
