...
import mone.api.pathing.goals.*;
import mone.api.process.IMineProcess;
 ...
 *
 */
public final class MineProcess extends MoneProcessHelper implements IMineProcess {
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (calcFailed) {
            if (!knownOreLocations.isEmpty() && MoneAPI.getSettings().blacklistClosestOnFailure.value) {
                logDirect("Unable to find any path to " + filter + ", blacklisting presumably unreachable closest instance...");
                if (MoneAPI.getSettings().notificationOnMineFail.value) {
        ...
        updateLoucaSystem();
        int mineGoalUpdateInterval = MoneAPI.getSettings().mineGoalUpdateInterval.value;
        List<BlockPos> curr = new ArrayList<>(knownOreLocations);
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            CalculationContext context = new CalculationContext(Mone, true);
            Mone.getExecutor().execute(() -> rescan(curr, context));
        }
        if (MoneAPI.getSettings().legitMine.value) {
            if (!addNearby()) {
                cancel();
                return null;
            }
        ...
        if (shaft.isPresent() && ctx.player().onGround()) {
            ...
            BlockState state = Mone.bsi.get0(pos);
            if (!MovementHelper.avoidBreaking(Mone.bsi, pos.getX(), pos.getY(), pos.getZ(), state)) {
                Optional<Rotation> rot = RotationUtils.reachable(ctx, pos);
                if (rot.isPresent() && isSafeToCancel) {
                    Mone.getLookBehavior().updateTarget(rot.get(), true);
    ...
    private void updateLoucaSystem() {
        ...
        ctx.getSelectedBlock().ifPresent(pos -> {
            if (knownOreLocations.contains(pos)) {
                copy.put(pos, System.currentTimeMillis() + MoneAPI.getSettings().mineDropLoiterDurationMSThanksLouca.value);
            }
        });
    ...
    private PathingCommand updateGoal() {
        ...
        boolean legit = MoneAPI.getSettings().legitMine.value;
        List<BlockPos> locs = knownOreLocations;
        if (!locs.isEmpty()) {
            CalculationContext context = new CalculationContext(Mone);
            List<BlockPos> locs2 = prune(context, new ArrayList<>(locs), filter, MoneAPI.getSettings().mineMaxOreLocationsCount.value, blacklist, droppedItemsScan());
        ...
        if (!legit && !MoneAPI.getSettings().exploreForBlocks.value) {
            return null;
        }
        // only when we should explore for blocks or are in legit mode we do this
        int y = MoneAPI.getSettings().legitMineYLevel.value;
        if (branchPoint == null) {
            /*if (!Mone.getPathingBehavior().isPathing() && playerFeet().y == y) {
    ...
    private void rescan(List<BlockPos> already, CalculationContext context) {
        ...
        List<BlockPos> locs = searchWorld(context, filter, MoneAPI.getSettings().mineMaxOreLocationsCount.value, already, blacklist, dropped);
        locs.addAll(dropped);
        if (locs.isEmpty() && !MoneAPI.getSettings().exploreForBlocks.value) {
    ...
    private boolean internalMiningGoal(BlockPos pos, CalculationContext context, List<BlockPos> locs) {
        ...
        if (MoneAPI.getSettings().internalMiningAirException.value && state.getBlock() instanceof AirBlock) {
            return true;
        }
    ...
    private Goal coalesce(BlockPos loc, List<BlockPos> locs, CalculationContext context) {
        boolean assumeVerticalShaftMine = !(Mone.bsi.get0(loc.above()).getBlock() instanceof FallingBlock);
        if (!MoneAPI.getSettings().forceInternalMining.value) {
    ...
    public List<BlockPos> droppedItemsScan() {
        if (!MoneAPI.getSettings().mineScanDroppedItems.value) {
    ...
    public static List<BlockPos> searchWorld(CalculationContext ctx, BlockOptionalMetaLookup filter, int max, List<BlockPos> alreadyKnown, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        for (BlockOptionalMeta bom : filter.blocks()) {
            ...
            if (CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.contains(block)) {
                BetterBlockPos pf = ctx.Mone.getPlayerContext().playerFeet();

                // maxRegionDistanceSq 2 means adjacent directly or adjacent diagonally; nothing further than that
                locs.addAll(ctx.worldData.getCachedWorld().getLocationsOf(
                        BlockUtils.blockToString(block),
                        MoneAPI.getSettings().maxCachedWorldScanCount.value,
                        pf.x,
                        pf.z,
                        2
                ));
        ...
        if (!untracked.isEmpty() || (MoneAPI.getSettings().extendCacheOnThreshold.value && locs.size() < max)) {
            locs.addAll(MoneAPI.getProvider().getWorldScanner().scanChunkRadius(
    ...
    private boolean addNearby() {
            ...
            for (int y = playerFeet.getY() - searchDist; y <= playerFeet.getY() + searchDist; y++) {
                for (int z = playerFeet.getZ() - searchDist; z <= playerFeet.getZ() + searchDist; z++) {
                    ...
                    if (filter.has(bsi.get0(x, y, z))) {
                        ...
                        if ((MoneAPI.getSettings().legitMineIncludeDiagonals.value && knownOreLocations.stream().anyMatch(ore -> ore.distSqr(pos) <= 2 /* sq means this is pytha dist <= sqrt(2) */)) || RotationUtils.reachable(ctx, pos, fakedBlockReachDistance).isPresent()) {
        ...
        knownOreLocations = prune(new CalculationContext(Mone), knownOreLocations, filter, MoneAPI.getSettings().mineMaxOreLocationsCount.value, blacklist, dropped);
        return true;
    }
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter, int max, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        List<BlockPos> locs = locs2
                ...
                .filter(pos -> {
                    if (MoneAPI.getSettings().allowOnlyExposedOres.value) {
                ...
                .filter(pos -> pos.getY() >= MoneAPI.getSettings().minYLevelWhileMining.value + ctx.world.dimensionType().minY())

                .filter(pos -> pos.getY() <= MoneAPI.getSettings().maxYLevelWhileMining.value)
    ...
    public static boolean isNextToAir(CalculationContext ctx, BlockPos pos) {
        int radius = MoneAPI.getSettings().allowOnlyExposedOresDistance.value;
    ...
    public void mine(int quantity, BlockOptionalMetaLookup filter) {
        ...
        if (filter != null) {
            rescan(new ArrayList<>(), new CalculationContext(Mone));
        }
    }
    ...
    private BlockOptionalMetaLookup filterFilter() {
        ...
        if (!MoneAPI.getSettings().allowBreak.value) {
            BlockOptionalMetaLookup f = new BlockOptionalMetaLookup(this.filter.blocks()
                    .stream()
                    .filter(e -> MoneAPI.getSettings().allowBreakAnyway.value.contains(e.getBlock()))
