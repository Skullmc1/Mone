...
public final class InventoryBehavior extends Behavior implements Helper {
    ...
    public InventoryBehavior(Mone Mone) {
        super(Mone);
    }
    ...
    private boolean requestSwapWithHotBar(int inInventory, int inHotbar) {
        ...
        if (ticksSinceLastInventoryMove < MoneAPI.getSettings().ticksBetweenInventoryMoves.value) {
            logDebug("Inventory move requested but delaying " + ticksSinceLastInventoryMove + " " + MoneAPI.getSettings().ticksBetweenInventoryMoves.value);
            return false;
        }
        if (MoneAPI.getSettings().inventoryMoveOnlyIfStationary.value && !Mone.getInventoryPauserProcess().stationaryForInventoryMove()) {
    ...
    private int firstValidThrowaway() { // TODO offhand idk
        ...
        for (int i = 0; i < invy.size(); i++) {
            if (MoneAPI.getSettings().acceptableThrowawayItems.value.contains(invy.get(i).getItem())) {
                return i;
            }
        }
        return -1;
    ...
    private int bestToolAgainst(Block against) {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (MoneAPI.getSettings().itemSaver.value && (stack.getDamageValue() + MoneAPI.getSettings().itemSaverThreshold.value) >= stack.getMaxDamage() && stack.getMaxDamage() > 1) {
                continue;
            }
    ...
    public boolean hasGenericThrowaway() {
        for (Item item : MoneAPI.getSettings().acceptableThrowawayItems.value) {
    ...
    public boolean selectThrowawayForLocation(boolean select, int x, int y, int z) {
        BlockState maybe = Mone.getBuilderProcess().placeAt(x, y, z, Mone.bsi.get0(x, y, z));
    ...
    public boolean throwaway(boolean select, Predicate<? super ItemStack> desired) {
        return throwaway(select, desired, MoneAPI.getSettings().allowInventory.value);
    }
