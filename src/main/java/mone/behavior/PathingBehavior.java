...
import mone.Mone;
import mone.api.behavior.IPathingBehavior;
...
import mone.api.utils.PathCalculationResult;
import mone.api.utils.interfaces.IGoalRenderPos;
import mone.pathing.calc.AStarPathFinder;
...
import mone.utils.pathing.Favoring;
...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public PathingBehavior(Mone Mone) {
        super(Mone);
    }
    ...
    private void dispatchEvents() {
        ...
        for (PathEvent event : curr) {
            Mone.getGameEventHandler().onPathEvent(event);
        }
    }
    ...
    public void onTick(TickEvent event) {
        ...
        if (event.getType() == TickEvent.Type.OUT) {
            ...
            Mone.getPathingControlManager().cancelEverything();
            return;
        }

        expectedSegmentStart = pathStart();
        Mone.getPathingControlManager().preTick();
        tickPath();
        ticksElapsedSoFar++;
        dispatchEvents();
    }
    ...
    private void tickPath() {
        ...
        if (pauseRequestedLastTick && safeToCancel) {
            ...
            if (unpausedLastTick) {
                ...
                Mone.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
            }
        ...
        synchronized (pathPlanLock) {
            ...
            if (MoneAPI.getSettings().splicePath.value) {
            ...
            synchronized (pathCalcLock) {
                ...
                if (ticksRemainingInSegment(false).get() < MoneAPI.getSettings().planningTickLookahead.value) {
    ...
    public boolean secretInternalSetGoalAndPath(PathingCommand command) {
        ...
        if (command instanceof PathingCommandContext) {
            ...
            context = new CalculationContext(Mone, true);
        }
        if (goal == null) {
            return false;
        }
    ...
    public boolean isSafeToCancel() {
        if (current == null) {
            return !Mone.getElytraProcess().isActive() || Mone.getElytraProcess().isSafeToCancel();
        }
        return safeToCancel;
    }
    ...
    public boolean cancelEverything() {
        ...
        Mone.getPathingControlManager().cancelEverything(); // regardless of if we can stop the current segment, we can still stop the processes
        return doIt;
    }
    ...
    private void findPathInNewThread(final BlockPos start, final boolean talkAboutIt, CalculationContext context) {
        ...
        long failureTimeout;
        if (current == null) {
            primaryTimeout = MoneAPI.getSettings().primaryTimeoutMS.value;
            failureTimeout = MoneAPI.getSettings().failureTimeoutMS.value;
        } else {
            primaryTimeout = MoneAPI.getSettings().planAheadPrimaryTimeoutMS.value;
            failureTimeout = MoneAPI.getSettings().planAheadFailureTimeoutMS.value;
        }
    ...
    private AbstractNodeCostSearch createPathfinder(BlockPos start, Goal goal, IPath previous, CalculationContext context) {
        ...
        if (MoneAPI.getSettings().simplifyUnloadedYCoord.value && goal instanceof IGoalRenderPos) {
