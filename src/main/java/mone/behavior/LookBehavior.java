...
import mone.api.behavior.ILookBehavior;
...
import mone.behavior.look.ForkableRandom;
...
public final class LookBehavior extends Behavior implements ILookBehavior {
    ...
    public LookBehavior(Mone Mone) {
        super(Mone);
        this.processor = new AimProcessor(Mone.getPlayerContext());
        ...
        switch (event.getState()) {
            ...
            case POST: {
                ...
                if (this.prevRotation != null) {
                    ...
                    while (this.smoothYawBuffer.size() > MoneAPI.getSettings().smoothLookTicks.value) {
                        this.smoothYawBuffer.removeFirst();
                    }
                    this.smoothPitchBuffer.addLast(this.target.rotation.getPitch());
                    while (this.smoothPitchBuffer.size() > MoneAPI.getSettings().smoothLookTicks.value) {
                    ...
                    } else if (ctx.player().isFallFlying() ? MoneAPI.getSettings().elytraSmoothLook.value : MoneAPI.getSettings().smoothLook.value) {
    ...
    public Optional<Rotation> getEffectiveRotation() {
        if (MoneAPI.getSettings().freeLook.value) {
    ...
    private static abstract class AbstractAimProcessor implements ITickableAimProcessor {
        ...
        public final void tick() {
            // randomLooking
            this.randomYawOffset = (this.rand.nextDouble() - 0.5) * MoneAPI.getSettings().randomLooking.value;
            this.randomPitchOffset = (this.rand.nextDouble() - 0.5) * MoneAPI.getSettings().randomLooking.value;
            ...
            this.randomYawOffset += random * MoneAPI.getSettings().randomLooking113.value;
        }
        ...
        enum Mode {
            ...
            static Mode resolve(IPlayerContext ctx, boolean blockInteract) {
                ...
                if (ctx.player().isFallFlying()) {
                    ...
                    // rotation needs to be set somehow, otherwise Mone will halt since objectMouseOver() will just be
