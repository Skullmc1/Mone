...
public class Avoidance {
    ...
    public static List<Avoidance> create(IPlayerContext ctx) {
        if (!MoneAPI.getSettings().avoidance.value) {
            return Collections.emptyList();
        }
        List<Avoidance> res = new ArrayList<>();
        double mobSpawnerCoeff = MoneAPI.getSettings().mobSpawnerAvoidanceCoefficient.value;
        double mobCoeff = MoneAPI.getSettings().mobAvoidanceCoefficient.value;
        if (mobSpawnerCoeff != 1.0D) {
            ctx.worldData().getCachedWorld().getLocationsOf("mob_spawner", 1, ctx.playerFeet().x, ctx.playerFeet().z, 2)
                    .forEach(mobspawner -> res.add(new Avoidance(mobspawner, mobSpawnerCoeff, MoneAPI.getSettings().mobSpawnerAvoidanceRadius.value)));
        }
        if (mobCoeff != 1.0D) {
            ctx.entitiesStream()
                    ...
                    .forEach(entity -> res.add(new Avoidance(entity.blockPosition(), mobCoeff, MoneAPI.getSettings().mobAvoidanceRadius.value)));
        }
        return res;
    }
