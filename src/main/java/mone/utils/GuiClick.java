...
public class GuiClick extends Screen implements Helper {
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        Vec3 far = toWorld(mx, my, 1); // "Use 0.945 that's what stack overflow says" - Qceee

        if (near != null && far != null) {
            Vec3 viewerPos = new Vec3(PathRenderer.posX(), PathRenderer.posY(), PathRenderer.posZ());
            LocalPlayer player = MoneAPI.getProvider().getPrimaryMone().getPlayerContext().player();
        ...
        if (currentMouseOver != null) { //Catch this, or else a click into void will result in a crash
            if (event.button() == 0) {
                if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                    MoneAPI.getProvider().getPrimaryMone().getSelectionManager().removeAllSelections();
                    MoneAPI.getProvider().getPrimaryMone().getSelectionManager().addSelection(BetterBlockPos.from(clickStart), BetterBlockPos.from(currentMouseOver));
                    MutableComponent component = Component.literal("Selection made! For usage: " + MoneAPI.getSettings().prefix.value + "help sel");
                    ...
                    MoneAPI.getProvider().getPrimaryMone().getCustomGoalProcess().setGoalAndPath(new GoalBlock(currentMouseOver));
                }
            } else if (event.button() == 1) {
                MoneAPI.getProvider().getPrimaryMone().getCustomGoalProcess().setGoalAndPath(new GoalBlock(currentMouseOver.above()));
            }
        }
        clickStart = null;
    ...
    public void onRender(PoseStack modelViewStack, Matrix4f projectionMatrix) {
        ...
        if (currentMouseOver != null) {
            ...
            if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                ...
                IRenderer.emitAABB(bufferBuilder, modelViewStack, new AABB(Math.min(a.x, b.x), Math.min(a.y, b.y), Math.min(a.z, b.z), Math.max(a.x, b.x) + 1, Math.max(a.y, b.y) + 1, Math.max(a.z, b.z) + 1), MoneAPI.getSettings().pathRenderLineWidthPixels.value);
