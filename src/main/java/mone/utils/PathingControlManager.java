...
public class PathingControlManager implements IPathingControlManager {
    ...
    private final HashSet<IMoneProcess> processes; // unGh
    private final List<IMoneProcess> active;
    private IMoneProcess inControlLastTick;
    private IMoneProcess inControlThisTick;
    private PathingCommand command;

    public PathingControlManager(Mone Mone) {
        this.Mone = Mone;
        this.processes = new HashSet<>();
        this.active = new ArrayList<>();
        Mone.getGameEventHandler().registerEventListener(new AbstractGameEventListener() { // needs to be after all behavior ticks
            @Override
    ...
    @Override
    public void registerProcess(IMoneProcess process) {
    ...
    public void cancelEverything() { // called by PathingBehavior on TickEvent Type OUT
        ...
        command = null;
        active.clear();
        for (IMoneProcess proc : processes) {
            proc.onLostControl();
    ...
    @Override
    public Optional<IMoneProcess> mostRecentInControl() {
    ...
    public void preTick() {
        ...
        inControlThisTick = null;
        PathingBehavior p = Mone.getPathingBehavior();
    ...
    private void postTick() {
        ...
        switch (command.commandType) {
            case FORCE_REVALIDATE_GOAL_AND_PATH:
                ...
                break;
            case REVALIDATE_GOAL_AND_PATH:
                if (MoneAPI.getSettings().cancelOnGoalInvalidation.value && (command.goal == null || revalidateGoal(command.goal))) {
                    p.softCancelIfSafe();
                }
                p.secretInternalSetGoalAndPath(command);
                break;
    ...
    public boolean forceRevalidate(Goal newGoal) {
        PathExecutor current = Mone.getPathingBehavior().getCurrent();
        if (current != null) {
    ...
    public PathingCommand executeProcesses() {
        for (IMoneProcess process : processes) {
            if (process.isActive()) {
        ...
        active.sort(Comparator.comparingDouble(IMoneProcess::priority).reversed());

        Iterator<IMoneProcess> iterator = active.iterator();
        while (iterator.hasNext()) {
            IMoneProcess proc = iterator.next();

            PathingCommand exec = proc.onTick(Objects.equals(proc, inControlLastTick) && Mone.getPathingBehavior().calcFailedLastTick(), Mone.getPathingBehavior().isSafeToCancel());
            if (exec == null) {
                if (proc.isActive()) {
                ...
                if (!proc.isTemporary()) {
                    iterator.forEachRemaining(IMoneProcess::onLostControl);
                }
                return exec;
            }
        }
